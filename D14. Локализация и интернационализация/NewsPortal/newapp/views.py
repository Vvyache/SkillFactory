...
# Давайте попробуем сделать так, чтобы детали товара кэшировались до тех пор, пока они не изменятся.
# Для этого перейдём в sample_app/views.py, чтобы переопределить в DetailView метод получения объекта.
#     queryset = News.objects.all()

    def get_object(self, *args, **kwargs):  # переопределяем метод получения объекта, как ни странно
        obj = cache.get(f'news-{self.kwargs["pk"]}',
                        None)  # кэш очень похож на словарь,
